...
- **Select** property
  ...
  - If you want to import a **Multiple Select** property with multiple thesauri values, use the pipe symbol | as the separator between your values and do not include spaces between each value (e.g.  ```Arbitrary arrest|Illegal search|Enforced disappearance```). You may include grouped term same way as importing single values (e.g. ```Asia::Bangladesh|Asia::Cambodia|Asia::India```).
